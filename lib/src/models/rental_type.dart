...
enum RentalType {
  ...
  const RentalType(this.apiValue, this.label, this.priceUnit);
...
class RentalTypeChips extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Widget chip(String label, RentalType? type, IconData icon) => ChoiceChip(
          avatar: Icon(icon,
              size: 18,
              color: value == type ? Colors.white : IvoryColors.green),
          label: Text(label),
          selected: value == type,
          showCheckmark: false,
          ...
          labelStyle: TextStyle(
              ...
              color: value == type ? Colors.white : IvoryColors.ink),
          onSelected: (_) => onChanged(type),
        );
    return Wrap(spacing: 8, runSpacing: 8, children: [
      chip('Toutes durées', null, Icons.all_inclusive),
      chip(
